...
const SocialLink: React.FC<SocialLinkProps> = ({ href, bgColor, icon, name }) => (
  <div className="relative group">
    ...
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-dark-bg text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10">
      {name}
      <svg className="absolute text-dark-bg h-2 w-full left-0 top-full" x="0px" y="0px" viewBox="0 0 255 255" xmlSpace="preserve"><polygon className="fill-current" points="0,0 127.5,127.5 255,0"/></svg>
    </div>
  </div>
);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden text-center transform hover:-translate-y-2 transition-transform duration-300 flex flex-col h-full">
      ...
      <div className="p-4 bg-gradient-to-t from-amber-500 to-amber-400 flex-grow flex flex-col justify-center">
        <h3 className="text-xl font-bold text-dark-bg font-display">{name}</h3>
