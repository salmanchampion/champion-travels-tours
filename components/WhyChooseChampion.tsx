...
const FeatureCard: React.FC<{ section: WhyChooseSection }> = ({ section }) => (
  <div className="bg-light-bg p-8 rounded-lg text-center transform hover:-translate-y-2 transition-transform duration-300 shadow-lg hover:shadow-primary/20">
    ...
    <p className="text-muted-text">{section.description}</p>
  </div>
);
        ...
        <section className="py-20 bg-dark-bg">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12">
                    <h2 className="text-4xl md:text-5xl font-display font-bold text-primary">{content.title}</h2>
                    <p className="mt-4 text-lg text-muted-text max-w-3xl mx-auto">{content.subtitle}</p>
                    <div className="mt-6">
                        <button 
                            onClick={toggleLanguage}
                            className="bg-secondary text-dark-bg font-bold py-2 px-6 rounded-full hover:bg-amber-600 transition-all duration-300"
                        >
