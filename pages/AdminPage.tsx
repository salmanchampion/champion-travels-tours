...
const AdminInput = ({ label, name, value, onChange, placeholder = '', type = 'text' }: { label: string, name: string, value: any, onChange: (e: ChangeEvent<HTMLInputElement>) => void, placeholder?: string, type?: string }) => (
    <div>
    ...
    const handleNestedChange = (path: string, value: any) => {
        setLocalData(prev => {
            ...
            for (let i = 0; i < keys.length - 1; i++) {
                const key = keys[i];
                if (!isNaN(Number(keys[i+1]))) { // It's an array
                     ...
                     {localData.pages.team.talentedEmployees.map((member, index) => (
                        <div key={index} className="mb-4 p-3 border border-gray-700 rounded-md">
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <button onClick={() => removeListItem('pages.team.talentedEmployees', index)} className="bg-red-600 text-white px-3 py-1 rounded self-end h-10">Remove</button>
                            </div>
                        </div>
                     ))}
                     <button onClick={() => addListItem('pages.team.talentedEmployees', { name: 'New Member', role: 'Role', imageUrl: '' })} className="mt-2 bg-green-600 text-white font-bold py-2 px-4 rounded">Add Employee</button>
                </Section>
                
                <Section title="Why Choose Us Page">
                    <h4 className="font-bold text-xl mb-2 text-secondary">Expert Guides Section</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <AdminInput label="Tagline" name="pages.whyChooseUs.guides.tagline" value={localData.pages.whyChooseUs.guides.tagline} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminInput label="Title" name="pages.whyChooseUs.guides.title" value={localData.pages.whyChooseUs.guides.title} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminTextarea label="Description" name="pages.whyChooseUs.guides.description" value={localData.pages.whyChooseUs.guides.description} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminInput label="Subheading" name="pages.whyChooseUs.guides.subheading" value={localData.pages.whyChooseUs.guides.subheading} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminTextarea label="Sub-description" name="pages.whyChooseUs.guides.subDescription" value={localData.pages.whyChooseUs.guides.subDescription} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminInput label="Button Text" name="pages.whyChooseUs.guides.buttonText" value={localData.pages.whyChooseUs.guides.buttonText} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminInput label="Main Image URL" name="pages.whyChooseUs.guides.mainImage" value={localData.pages.whyChooseUs.guides.mainImage} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                        <AdminInput label="Secondary Image URL" name="pages.whyChooseUs.guides.secondaryImage" value={localData.pages.whyChooseUs.guides.secondaryImage} onChange={e => handleNestedChange(e.target.name, e.target.value)} />
                    </div>
                </Section>
